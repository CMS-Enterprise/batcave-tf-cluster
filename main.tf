...
resource "aws_autoscaling_attachment" "eks_managed_node_groups_alb_attachment" {
 count = length(module.eks_managed_node_groups)
 autoscaling_group_name = lookup(module.eks_managed_node_groups.node_group_autoscaling_group_names[count.index], "name", null)
